...
/*void bitstreamXOR(char * a, char * b, char * output)
{
  // Input: 32bits Return: 32bits
  int i;
...
unsigned long long bitstreamXOR(unsigned long long a, unsigned long long b)
{
   return a ^ b;
   /*int i;
...
void functionF(unsigned int A, unsigned long long J)
{
   // A (32) J(48)
   ...
   // Faz o xor
   unsigned long long xorExpanded = bitstreamXOR(expanded, J);
   // Divide E(A) em B0...B8 (de 6 bits)
   //printf("T=%llX\nT'=%llx\n",expanded, xorExpanded);
}
...
void DESKeySchedule(unsigned long long key, unsigned long long* subKeys)
{
   ...
   long long mask = 0xFEFEFEFEFEFEFEFE;
   long long resultado = key & mask; // Resultado em 64 bits
   printf("%llX %llX %llX\n", key, mask, resultado);
